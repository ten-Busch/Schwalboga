(function () {
  ...
  function allReplays() {
    return displayOnlyReplay ? [displayOnlyReplay] : [];
  }
  ...
  function replayFileUrl() {
    return `replays/${encodeURIComponent(defaultReplayFile)}`;
  }
  ...
  function renderDetail(replay) {
    ...
    const replayUrl = replayFileUrl();
    detail.innerHTML = `
      <header class="replay-detail-header">
        <div><p class="eyebrow">${esc(replay.meta?.format || 'Showdown Battle')}</p><h3>${esc(replayTitle(replay))}</h3><p>${esc(dateLabel(replay.meta?.date))} · ${Number(replay.meta?.turns || 0)} Runden</p></div>
