...
function loadDefaultLists() {
	$(".set-selector").select2({
		formatResult: function (object) {
			if (window.SCHWALBOGA_CALC && typeof window.SCHWALBOGA_CALC.formatSetOption === "function") {
				return window.SCHWALBOGA_CALC.formatSetOption(object);
			}
			if ($("#randoms").prop("checked")) {
		...
		query: function (query) {
			...
			if (window.SCHWALBOGA_CALC && typeof window.SCHWALBOGA_CALC.filterSetOptions === "function") {
				options = window.SCHWALBOGA_CALC.filterSetOptions(options, $(this.element).closest(".poke-info").prop("id"));
			}
			for (var i = 0; i < options.length; i++) {
				var option = options[i];
				var pokeName = option.pokemon.toUpperCase();
				var displayName = (option.displayPokemon || option.displayText || "").toUpperCase();
				if (!query.term || query.term.toUpperCase().split(" ").every(function (term) {
					return pokeName.indexOf(term) === 0 || pokeName.indexOf("-" + term) >= 0 || pokeName.indexOf(" " + term) >= 0 ||
						displayName.indexOf(term) === 0 || displayName.indexOf("-" + term) >= 0 || displayName.indexOf(" " + term) >= 0;
				})) {
					if ($("#randoms").prop("checked")) {
		...
		initSelection: function (element, callback) {
			var firstOption = getFirstValidSetOption();
			if (window.SCHWALBOGA_CALC && typeof window.SCHWALBOGA_CALC.getInitialSetOption === "function") {
				firstOption = window.SCHWALBOGA_CALC.getInitialSetOption(firstOption, $(element).closest(".poke-info").prop("id"));
			}
